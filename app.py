...
def highlight_auto():
    try:
        ...
        chapter_path = f"static/text/{book}/{chapter}.txt"
        if not os.path.exists(chapter_path):
            return jsonify({'error': 'Chapter text file not found'}), 404

        with open(chapter_path, "r", encoding="utf-8") as f:
            full_text = f.read()

        matches = detect_highlights(full_text, category)
        print(f"[AUTO-HIGHLIGHT] {len(matches)} matches for '{category}'")

        for match in matches:
            highlight_text = full_text[match['start']:match['end']]
