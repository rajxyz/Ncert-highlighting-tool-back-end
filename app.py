...
def get_chapter_highlights(book, chapter):
    try:
        ...
        print(f"[GET HIGHLIGHTS] 🎯 {len(highlights)} highlights returned")
...
def highlight_auto():
    try:
        ...
        print(f"[AUTO-HIGHLIGHT] 🧠 Detected {len(matches)} matches for category '{category}'")
        ...
        for match in matches:
            ...
            if text and start is not None and end is not None:
                save_detected_highlight(book, chapter, text, start, end, category, page_number, match_id, rule_name, source)
                valid_count += 1
            else:
                print(f"⚠️ Skipped invalid match: {match}")

        highlights = get_highlights(book, chapter)
        return jsonify({
            'message': f"{valid_count} highlight(s) saved",
...
def save_all_highlights():
    try:
        ...
        print(f"[SAVE ALL] 💾 Saved {len(highlights)} highlights to {save_path}")
...
# ✅ Get all highlights for chapter (with filters)
@app.route('/api/chapter_highlights/<book>/<chapter>')
def get_chapter_highlights(book, chapter):
    try:
        page_number = request.args.get('page_number')
        ...
        all_highlights = get_highlights(book, chapter)
        print(f"[GET HIGHLIGHTS] 🎯 Total highlights loaded: {len(all_highlights)}")

        # Filtering
        if page_number is not None:
            all_highlights = [h for h in all_highlights if str(h.get('page_number')) == str(page_number)]
            print(f"  ↪️ Filtered by page_number={page_number}: {len(all_highlights)}")

        if category:
            all_highlights = [h for h in all_highlights if h.get('category') == category]
            print(f"  ↪️ Filtered by category='{category}': {len(all_highlights)}")
...
def highlight_auto():
    try:
        ...
        highlights = get_highlights(book, chapter)
        print(f"✅ Highlights after saving: {len(highlights)}")
